doctor_types = [ 
    {
        'id': 'doctor_type1',
        'type' : 'Primary Care Physician (PCP)'
    },
    {
        'id': 'doctor_type2',
        'type' : 'OB-GYN (Obstetrician-Gynecologist)'
    },
    {
        'id': 'doctor_type3',
        'type' : 'Dermatologist'
    },
    {
        'id': 'doctor_type4',
        'type' : 'Dentist'
    },
    {
        'id': 'doctor_type5',
        'type' : 'Ear, Nose &amp; Throat Doctor (ENT / Otolaryngologist)'
    },
    {
        'id': 'doctor_type6',
        'type' : 'Eye Doctor'
    },
    {
        'id': 'doctor_type7',
        'type' : 'Psychiatrist'
    },
    {
        'id': 'doctor_type8',
        'type' : 'Orthopedic Surgeon (Orthopedist)'
    },
    {
        'id': 'doctor_type9',
        'type' : 'Acupuncturist'
    },
    {
        'id': 'doctor_type10',
        'type' : 'Allergist (Immunologist)'
    },
    {
        'id': 'doctor_type11',
        'type' : 'Audiologist'
    },
    {
        'id': 'doctor_type12',
        'type' : 'Cardiologist (Heart Doctor)'
    },
    {
        'id': 'doctor_type13',
        'type' : 'Cardiothoracic Surgeon'
    },
    {
        'id': 'doctor_type14',
        'type' : 'Chiropractor'
    },
    {
        'id': 'doctor_type15',
        'type' : 'Colorectal Surgeon'
    },
    {
        'id': 'doctor_type18',
        'type' : 'Dietitian / Nutritionist'
    },
    {
        'id': 'doctor_type20',
        'type' : 'Endocrinologist (incl Diabetes Specialists)'
    },
    {
        'id': 'doctor_type22',
        'type' : 'Gastroenterologist'
    },
    {
        'id': 'doctor_type23',
        'type' : 'Geriatrician'
    },
    {
        'id': 'doctor_type24',
        'type' : 'Hearing Specialist'
    },
    {
        'id': 'doctor_type25',
        'type' : 'Hematologist (Blood Specialist)'
    },
    {
        'id': 'doctor_type26',
        'type' : 'Infectious Disease Specialist'
    },
    {
        'id': 'doctor_type27',
        'type' : 'Infertility Specialist'
    },
    {
        'id': 'doctor_type28',
        'type' : 'Midwife'
    },
    {
        'id': 'doctor_type29',
        'type' : 'Naturopathic Doctor'
    },
    {
        'id': 'doctor_type30',
        'type' : 'Nephrologist (Kidney Specialist)'
    },
    {
        'id': 'doctor_type31',
        'type' : 'Neurologist (incl Headache Specialists)'
    },
    {
        'id': 'doctor_type32',
        'type' : 'Neurosurgeon'
    },
    {
        'id': 'doctor_type34',
        'type' : 'Oncologist'
    },
    {
        'id': 'doctor_type35',
        'type' : 'Ophthalmologist'
    },
    {
        'id': 'doctor_type36',
        'type' : 'Optometrist'
    },
    {
        'id': 'doctor_type37',
        'type' : 'Oral Surgeon'
    },
    {
        'id': 'doctor_type38',
        'type' : 'Orthodontist'
    },
    {
        'id': 'doctor_type40',
        'type' : 'Pain Management Specialist'
    },
    {
        'id': 'doctor_type41',
        'type' : 'Pediatric Dentist'
    },
    {
        'id': 'doctor_type42',
        'type' : 'Pediatrician'
    },
    {
        'id': 'doctor_type43',
        'type' : 'Physiatrist (Physical Medicine)'
    },
    {
        'id': 'doctor_type44',
        'type' : 'Physical Therapist'
    },
    {
        'id': 'doctor_type45',
        'type' : 'Plastic Surgeon'
    },
    {
        'id': 'doctor_type46',
        'type' : 'Podiatrist (Foot and Ankle Specialist)'
    },
    {
        'id': 'doctor_type48',
        'type' : 'Prosthodontist'
    },
    {
        'id': 'doctor_type50',
        'type' : 'Psychologist'
    },
    {
        'id': 'doctor_type51',
        'type' : 'Pulmonologist (Lung Doctor)'
    },
    {
        'id': 'doctor_type52',
        'type' : 'Radiologist'
    },
    {
        'id': 'doctor_type53',
        'type' : 'Rheumatologist'
    },
    {
        'id': 'doctor_type54',
        'type' : 'Sleep Medicine Specialist'
    },
    {
        'id': 'doctor_type55',
        'type' : 'Sports Medicine Specialist'
    },
    {
        'id': 'doctor_type56',
        'type' : 'Surgeon'
    },
    {
        'id': 'doctor_type57',
        'type' : 'Therapist / Counselor'
    },
    {
        'id': 'doctor_type58',
        'type' : 'Urgent Care Specialist'
    },
    {
        'id': 'doctor_type59',
        'type' : 'Urological Surgeon'
    },
    {
        'id': 'doctor_type60',
        'type' : 'Urologist'
    },
    {
        'id': 'doctor_type61',
        'type' : 'Vascular Surgeon'
    },
    {
        'id': 'doctor_type62',
        'type' : 'Endodontist'
    },
    {
        'id': 'doctor_type63',
        'type' : 'Periodontist'
    }
]